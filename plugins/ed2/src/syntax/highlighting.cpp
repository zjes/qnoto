...
struct State
{
    ...
    {
        //qDebug() << "switch state:" << contexts.top()->name() << "->" << ctx;
...
void Highlighting::highlightBlock(TextBlock& block)
{
    ...
    if (m_definition && !m_definition->isEmpty()){
        ...
        QString lastState = data->state->contexts.last()->name();
        highlightLine(block, data);
        block.stateChanged = data->state->contexts.last()->name() != lastState;//nextData && data->state->contexts.last() != nextData->state->contexts.first();
    }
...
void Highlighting::highlightLine(TextBlock& block, UserData* data)
{
    ...
    while(offset < text.length()){
        ...
        for(const auto& rule: ctx->rules()){
            ...
            if(nctx.isEmpty()){
                nctx = ctx->name();
                applyFormat(block, ctx->attribute(), beginOffset, newOffset-beginOffset);
...
bool Highlighting::paintBlock(const QTextBlock& block, QPainter& painter, const QRect& bounding)
{
    ...
    if (m_markWhite){
        ...
        for(const auto& ch: block.text()){
            if (ch == ' '){
                QRectF pr = QRectF(bounding.left()+line.cursorToX(i), bounding.top(), 5, bounding.height());
                painter.drawText(pr, ".");
            }
        }
    }
